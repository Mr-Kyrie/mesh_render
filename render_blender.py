...
scene.render.resolution_x = 274#设置渲染分辨率
...
cam_constraint.target = b_empty#目标物体
camera_loc = []
model_identifier = os.path.split(os.path.split(args.obj)[0])[1]
fp = os.path.join(args.output_folder, model_identifier, model_identifier)
scene.render.image_settings.file_format = 'PNG'  # set output format to .png

camera_params()#生成相机参数
params = np.loadtxt('H:/rendering_metadata.txt', delimiter=' ')#读取相机参数文件
